...
MODULE_DESCRIPTION("spi LKM spi module driver to access RFID ");

//1. creating all items of a device driver as needed.
#define Device_name "RFID_SPI_DRV"
#define RC522_CHIP_ID_REG 0x37 //  chip version
...
static int __init ModuleInit(void)
{
  ...
  printk(KERN_INFO "sPI MODULE STARTING....\n");

  // PARAMETER OF SPI DEVICE
    static struct spi_board_info rfid_board_info = {
      ...
      if(!master){
        printk(KERN_ALERT "THERE IS no SPI BUS WITH %d\n", SPI_BUS_USED);
         return -ENODEV;
      }
      //create new spi device
      rfid_spi_device = spi_new_device(master, &rfid_board_info);
      if(!rfid_spi_device){
        printk("could not create a new spi device...");
        put_device(&master->dev); // Release the master device
         return -ENODEV;
       // return -1;
      }
      rfid_spi_device -> bits_per_word = 8;

      //setup bus for device's parameters
      if(spi_setup(rfid_spi_device) != 0)
      {
        printk("could not change bus setup!\n");
        put_device(&master->dev); // Release master
        return -ENODEV;
      }
      //read chip I
        // Read chip ID from RC522

      u8 tx_buf[1] = { RC522_CHIP_ID_REG };
      u8 rx_buf[1] = { 0 };
      //int ret = spi_write_then_read(rfid_spi_device, tx_buf, 1, rx_buf, 1);
      int ret  =  spi_w8r8(rfid_spi_device, RC522_CHIP_ID_REG);
        if (ret) {
            printk(KERN_ALERT "SPI read failed: %d\n", ret);
         } else {
            printk(KERN_INFO "RC522 Chip ID: 0x%02X\n", rx_buf[0]);
        }
...
static void __exit ModuleExit(void){
   if (rfid_spi_device)
    ...
    printk(KERN_INFO "BYE SPI MODULE STOPED....\n");
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("ROBA");
//MODULE_DESCRIPTION("SPI RC522 RFID Kernel Module");
